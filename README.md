...
- データセンターがない場合と建設した場合を、同じ人口・水供給・季節条件で比較します
- データセンターの設備IT負荷を40〜300 MWから選び、住民向け上水が何MWから不足するか確認できます
- 今回の仮定では、124.6 MWまでは期間内不足なし、124.7 MWから不足が始まります
- 250 MWではD39から不足し、住民向け上水が102日累計で153.53 ML、約7%減ります
- 再生水の割合や水の配り方を変更した別ケースも一括比較できます

## 設計上の特徴

- 水量はすべてPythonで計算し、LLMには計算させません
- 「DCなし」と「DCあり」の差だけを、データセンターによる追加影響として扱います
- 上水の取水量、蒸発による消費、返流水などを分けて記録します
- LLMは住民・自治体・水道・DC事業者の判断だけを担当します
- 計算結果、プロンプト、未加工のLLM応答を保存し、後から検証できます
...
| 画面で選ぶ条件 | 冷却で地域へすぐ戻らない上水 | 配水バッファ | 住民累積不足 |
|---|---:|---:|---:|
| DCなし | 0 ML | 期末27.39 ML | 0 ML |
| DCあり・124.6 MW | 132.63 ML | 期末0.075 ML | 0 ML |
| DCあり・124.7 MW | 132.73 ML | 102日目に空 | 0.068 ML／1日 |
| DCあり・250 MW | 240.15 ML | 39日目に空 | 153.53 ML／64日 |
...
必要なのはmacOS／Linux／WSLのPOSIX `sh`互換シェルとPython 3.11以降だけです。Pythonの外部パッケージ、仮想環境、`pip install`は不要です。リポジトリのルートで次を実行してください。

```bash
./run.sh
```

夏季102日の水収支、4者の判断、40〜300 MWの感度データを生成し、ゲーム風画面`outputs/quickstart/inzai-water-game.html`を開きます。ブラウザーを自動で開けない環境では、最後に表示されるHTMLの絶対パスを開いてください。実際に選ばれたLLMも端末、画面、`outputs/quickstart/run-status.json`へ明記します。

DS4は任意です。`.env`がなくても固定seedの`MockProvider`で同じ画面まで必ず生成できます。すでに起動済みのDS4を使う場合だけ、`.env.example`を`.env`へコピーして接続先を確認してください。
...
`DS4_BASE_URL`が空でなければ`./run.sh`はDS4を試します。DS4が未起動、到達不能、タイムアウト、またはOpenAI互換の応答形式を返さなかった場合は、DS4とモックを途中で混在させず、同じseedのモックで102日間を最初から自動再実行します。APIキーはログや画面へ保存しません。DS4を必須にして接続失敗をエラーにしたい診断時だけ`./run.sh --provider ds4`を使います。

画面上の比較は`データセンターなし`と`データセンターあり`の2ケースです。DCありでは設備IT負荷を40〜300 MWの18段階から選べます。IT電力量、冷却用水需要、住民向け上水の減少、最初の不足日が連動し、固定条件では`124.6 MWまで不足なし`、`124.7 MWから不足開始`を並べて表示します。全表示点はPython水収支モデルで事前計算され、JavaScriptは結果を切り替えて表示するだけです。

## 個別の処理を実行する

通常は`./run.sh`だけで十分です。テスト、比較表、モック会議だけを個別に再生成したい場合は次を使えます。
